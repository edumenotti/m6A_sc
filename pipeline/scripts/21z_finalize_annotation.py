...
def main():
    ...
    with open(os.path.join(args.audit_dir, "final_annotation_audit.md"), "w") as fh:
        ...
        fh.write("   - `qc_low_score`: hemascribe_score < 0.03 — operational flag\n")
        fh.write("     (the 0.10 reference line drawn in 21d's score distribution\n")
        fh.write("     plot is the *visual* low-confidence reference; the 0.03 flag\n")
        fh.write("     is set tighter so EryP/MkP — which legitimately score low\n")
        fh.write("     against the HemaScribe HSPC-centric reference — are not\n")
        fh.write("     flagged en masse.)\n\n")
